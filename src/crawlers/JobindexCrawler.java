...
public class JobindexCrawler extends BaseCrawler {
    ...
    //constructor
    public JobindexCrawler(DI di, String URL, String area, String jobCategory, String foundAt) throws IOException {
                ...
                if (jobURL.contains(".pdf") || jobURL.contains(".ashx")) {
                    ...
                    try {
                        ...
                        if (!internalDocument.text().trim().isEmpty()) { // if page has text content
                            ...
                            if (isEnglish(textToDetect)) {// and if is english
                                danish = false;
                            } else {
                                danish = true;
                            }
    ...
    private ArrayList<Job> scanDocForExternal(Document doc, String tag, String city, String foundAt) throws ParseException {
        ...
        for (Element externalJobDiv : externalJobsDivs) {
            ...
            String jobText = "";
            try {
                jobText = externalJobDiv.text()
                        .substring(joblink.text().length() + 1, externalJobDiv.text().indexOf(" Gem job"))
                        .replaceAll(jobAnnouncer, "")
                        .replaceAll(place, "")
                        .replaceAll(Date, "")
                        .split("\\.\\.\\.")[0]
                        .split(" \\, ")[0];
            ...
            if (!isEnglish(jobText)) {
                ...
                try {
                    ...
                    e.printStackTrace(System.out);
                    METRICS.incrementExceptions();
                    // Something here failed, bad date format on site, so put previously used date
                    jobDate = previousJobDate;
                }
